...
| Markets and localization | PASS SAFE BASELINE / LAUNCH GATES | US remains active; Canada and the four-country EU market are draft; UK and Australia/New Zealand are draft. English is published; German, Spanish, French, and Dutch are untranslated, unassigned, and unpublished. CAD/EUR remain blocked by Shopify Payments setup; selectors, localized URLs, currencies, and per-region storefront behavior require activation-time regression. |
...
TASK-009 is `DONE`. The full criterion, event/schema, external-configuration, safety, and launch-gate reconciliation is in `reports/TASK-009-RESULT.md`; TASK-010 remains gated by TASK-007 and launch prerequisites.
