...
- TASK-017 root-cause, RED→GREEN, exact-width fixture, controlled development deployment/readback and protected-live evidence for the owner-reported Home visual defects.
- TASK-016 addendum preserving the owner's real desktop observations without inferring unsupplied viewport, console, accessibility or Theme Editor results.
...
- Home now keeps supplier-overlay/repeated-media surfaces disabled, including the two-Collection reference carousel confirmed incomplete at 390×844; custom Gallery/Trending/Offers cards distribute without empty desktop columns, Trending balances 2×2 below 990 px and the dark final CTA uses configured AA foreground contrast plus protected bottom spacing. No Product/global-media record changed.
...
- TASK-014/TASK-015 historically enabled seven evidence-backed reference sections; TASK-017 reduced the current state to three active and twelve disabled after real desktop and 390×844 Home asset-quality evidence. Fresh authenticated re-QA remains pending; live theme `192527597938` is unchanged.
